...
// Fetch all concerns for admin view
$stmt = $conn->prepare("SELECT c.id, u.username, c.title, c.status, c.created_at, c.updated_at FROM concerns c JOIN users u ON c.user_id = u.id ORDER BY c.created_at DESC");
$stmt->execute();
...
<head>
    ...
    <title>Admin Dashboard - PLMUN Concern Hub</title>
...
<body class="min-h-screen flex flex-col">
    <header class="flex items-center justify-between p-4 bg-white bg-opacity-10">
        ...
        <div class="flex items-center space-x-4">
            <i class="fas fa-user-circle text-3xl cursor-pointer" title="Admin Profile"></i>
            <a href="logout.php" class="underline">Logout</a>
        </div>
    </header>
    <main class="flex-grow p-6 max-w-6xl mx-auto">
        <h2 class="text-xl mb-4">All Concerns</h2>
                ...
                <tbody>
                    <?php foreach ($concerns as $concern): ?>
                        <tr class="border border-gray-300">
                            <td class="border border-gray-300 p-2"><?= htmlspecialchars($concern['username']) ?></td>
